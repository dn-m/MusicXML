...
class NotationsTests: XCTestCase {
    ...
    func testAccidentalMark() throws {
        ...
        """
        let _ = try MusicXMLDecoder().decode(AccidentalMark.self, from: xml)
    }

    func testTurn() throws {
        let xml = "<turn/>"
        let _ = try MusicXMLDecoder().decode(HorizontalTurn.self, from: xml)
    }
    ...
    func testStaccatoArticulations() throws {
        ...
        let decoded = try MusicXMLDecoder().decode(Articulations.self, from: xml)
    ...
    func testUpBowTechnical() throws {
        ...
        let decoded = try MusicXMLDecoder().decode(Technical.self, from: xml)
